// const qElem = document.getElementById("questionArea");
const scorecardElem = document.querySelector("#scorecard");
console.log(scorecardElem);
alert("Ready for a quiz?");
quiz();

function quiz() {
  let score = 0;
  let num;
  let question;
  let answer;
  let quiz_questions = [
    "How many moons does Earth have?",
    "How many moons does Saturn have?",
    "How many moons does Venus have?",
  ];
  let quiz_answers = [1, 82, 0];

  //get total number of questions
  let totalQuestion = quiz_questions.length;

  //generate random number for question
  num = Math.floor(Math.random() * 3);
  console.log(num);

  for (let count = 0; count < totalQuestion; count++) {
    question = quiz_questions[num];
    console.log(question);
    answer = prompt(question);
    if (answer == quiz_answers[num]) {
      console.log(quiz_answers[num]);
      score++;
      alert("Correct!");
    } else {
      console.log(quiz_answers[num]);
      alert("Wrong");
    }
    num++;
    if (num === totalQuestion) {
      num = 0;
    }
  }
  //   alert(`You got ${score} out of ${totalQuestion} questions correct.`);
  scorecardElem.textContent = `You got ${score} out of ${totalQuestion} questions correct.`;
}
